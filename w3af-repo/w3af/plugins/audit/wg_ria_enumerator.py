...
class wg_ria_enumerator(AuditPlugin):
    """
    ...
    def audit(self, freq, orig_response):
        ...
        url_generator = self._url_generator(self._target_url, self._extensions, self._wordlist_file)
    ...
    def _analyze_crossdomain_clientaccesspolicy(self, url, response, file_name):
        try:
            ...
            'cross-domain-access' in response.get_body():
                ...
                desc = 'The "{}" file at: "{}" is not a valid XML.'.format(file_name, response.get_url())
                i = Info('Invalid RIA settings file', desc, response.id, self.get_name())
                i.set_url(self._target_url)
                self.kb_append(self, 'wg_ria_enumerator', i)
        else:
            ...
            for url in url_list:
                ...
                if url == '*':
                    desc = 'The "{}" file at "{}" allows flash/silverlight access from any site. ' \
                           'Specify domain/uri in "{}" or just ' \
                           'remove that file if it does not make sense for you ' \
                           'and you will fix that issue.'.format(file_name, response.get_url(), file_name)
                    v = Vuln('Insecure RIA settings', desc, severity.LOW, response.id, self.get_name())
                    v.set_url(self._target_url)
                    self.kb_append(self, 'wg_ria_enumerator', v)
                else:
                    desc = 'The "{}" file at "{}" allows ' \
                           'flash/silverlight access from "{}".'.format(file_name, response.get_url(), url)
    ...
    def get_options(self):
        ...
        o = opt_factory('wordlist', self._wordlist_file, d, 'string')
        ol.add(o)
    ...
    def get_long_desc(self):
        ...
        """
        return 'This plugin searches for various Rich Internet Application files ' \
               '(check for <allow-access-from domain="*"/> inside of <cross-domain-policy></cross-domain-policy>). ' \
               'It is currently searches for:\n' \
               '* Google gears manifests ' \
               '(Files are used to determine which files are locally cached by google gears. ' \
               'They do not get cleared when the browser cache is cleared and may contain sensitive information);\n' \
               '* Flex crossdomain.xml ' \
               '(File stores domains which are allowed to make cross domain requests to the server);\n' \
               '* Silverlight clientaccesspolicy.xml ' \
               '(File determines which clients can access the server in place of the crossdomain.xml);'
