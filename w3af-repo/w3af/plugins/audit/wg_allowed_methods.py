...
class wg_allowed_methods(AuditPlugin):
    """
    ...
    DAV_METHODS = {'DELETE', 'PROPFIND', 'PROPPATCH', 'COPY', 'MOVE', 'LOCK', 'UNLOCK', 'MKCOL'}
    COMMON_METHODS = {'OPTIONS', 'GET', 'HEAD', 'POST', 'TRACE', 'PUT'}
    UNCOMMON_METHODS = {'*', 'SUBSCRIPTIONS', 'NOTIFY', 'DEBUG', 'TRACK', 'POLL', 'PIN', 'INVOKE', 'SUBSCRIBE', 'UNSUBSCRIBE'}
    # Methods taken from http://www.w3.org/Protocols/HTTP/Methods.html
    PROPOSED_METHODS = {'CHECKOUT', 'SHOWMETHOD', 'LINK', 'UNLINK', 'CHECKIN', 'TEXTSEARCH', 'SPACEJUMP', 'SEARCH', 'REPLY'}
    EXTRA_METHODS = {'CONNECT', 'RMDIR', 'MKDIR', 'REPORT', 'ACL', 'DELETE', 'INDEX', 'LABEL', 'INVALID'}
    VERSION_CONTROL = {'VERSION_CONTROL', 'CHECKIN', 'UNCHECKOUT', 'PATCH', 'MERGE', 'MKWORKSPACE', 'MKACTIVITY', 'BASELINE_CONTROL'}
    ...
    def audit(self, freq, orig_response):
        ...
        target_domain = freq.get_url()
        allowed_methods, id_list = self._identify_allowed_methods(target_domain)
        self._analyze_methods(target_domain, allowed_methods, id_list)
    ...
    def _identify_with_bruteforce(self, url):
        ...
        and get_response.get_body() == non_exist_response.get_body():
            desc = 'Host has a custom configuration, ' \
                   'in which any not implemented methods that are invoked are ' \
                   'defaulted to GET instead of returning a "Not Implemented" response.'
            response_ids = [non_exist_response.get_id(), get_response.get_id()]
            i = Info('Non existent methods default to GET', desc, response_ids, self._plugin_name)
            i.set_url(url)
    ...
    def _analyze_methods(self, url, allowed_methods, id_list):
        ...
        # Check for DAV
            ...
            # information
            desc = 'The URL "{}" has the following allowed methods. ' \
                   'These include DAV methods and should be disabled: {}.'.format(url, ', '.join(allowed_methods))
            i = Info('DAV methods enabled', desc, id_list, self._plugin_name)
            i.set_url(url)
            ...
            # REALLY use it !
            desc = 'The URL "{}" has the following enabled HTTP methods: {}.'.format(url, ', '.join(allowed_methods))
            i = Info('Allowed HTTP methods', desc, id_list, self._plugin_name)
            i.set_url(url)
            i['methods'] = allowed_methods
    ...
    def get_long_desc(self):
        ...
        """
        return 'This plugin finds which HTTP methods are enabled for a URI. ' \
               'The plugin will try to use the OPTIONS method to enumerate all available methods, ' \
